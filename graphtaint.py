...
def getYAMLFiles(path_to_dir):
    logger.info(f"Searching for YAML files in directory: {path_to_dir}")
    valid_  = [] 
    ...
    logger.info(f"Found {len(valid_)} YAML files.")
    return valid_ 
...
def getHelmTemplateContent(templ_dir):
    logger.info(f"Loading Helm template content from directory: {templ_dir}")
    ...
    logger.info(f"Loaded {len(template_content_dict)} template files.")
...
def getValidTaints(lis_template_matches): 
    ...
    files that are affected by the key 
    '''
    taint_lis = []
...
def mineSecretGraph(path2script, yaml_dict, secret_dict):
    logger.info(f"Mining secret graph for script: {path2script}")
    ...
    for k_, v_ in secret_dict.items():
        for tup_item in v_:
            for value in tup_item:
                ...
                compo_hiera_keys = [constants.DOT_SYMBOL.join(str_) for str_ in combinations(hierarchy_keys, 2)]
    ...
    logger.info(f"Found {len(valid_taints)} valid taints.")
...
def mineServiceGraph(script_path, dict_yaml, src_val):
    logger.info(f"Mining service graph for source value: {src_val}")
    ret_lis = [] 
    ...
    logger.info(f"Found {len(ret_lis)} service graph connections.")
    return ret_lis 

def mineNetPolGraph(script_, dict_y, src_val, src_keys):
    logger.info(f"Analyzing network policy connections for: {script_}")
    lis2ret = [] 
    ...
    logger.info(f"Network policy links found: {len(lis2ret)}")
    return lis2ret
